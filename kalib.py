    ...
    if "rata_pengukuran" in st.session_state:
            ...
            if st.button("Hitung Volume & Ketidakpastian"):
                ...
                #Ketidakpastian Diperluas
                    ...
                    st.write(f"U1 : **{(U1*Cs1)**2:.6f}**")
                    st.write(f"U2 : **{(U2*Cs2)**2:.6f}**")
                    st.write(f"U3 : **{(U3*Cs3)**2:.6f}**")
                    st.write(f"U4 : **{(U4*Cs4)**2:.6f}**")
                    st.write(f"U5 : **{(U5*Cs5)**2:.6f}**")
                    st.write(f"U6 : **{(U6*Cs6)**2:.6f}**")
                    ...
                    if koreksi < ketelitian_lb:
                        ...
                        st.write(f"Karena Nilai Koreksi ({koreksi:.5f}) Lebih Kecil Dari Ketelitian Labu Takar ({ketelitian_lb})")
                    else:
